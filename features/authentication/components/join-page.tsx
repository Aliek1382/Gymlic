...
import { useRouteParam } from "@/lib/route-params";
import {
  useAuthContext,
...
import { JoinSignedInForm } from "./join-signed-in-form";

export function JoinPage() {
  // /join/<code> is emitted once as a shell and the host rewrites every real
  // code onto it, so the code comes from the URL rather than route params.
  const code = useRouteParam();
  const { data: invitation, isPending: invitePending } = useInvitationPreview(
    code ?? ""
  );
  const { data: context, isPending: contextPending } = useAuthContext();

  if (code === null || invitePending || contextPending) return <RouteLoading />;
